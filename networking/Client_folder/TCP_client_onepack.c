...
int main(int argc, char *argv[])
{
    ...
    while((cvWaitKey(40) & 0xFF) != 'x')
    {
        ...
        //{ 
            ...
            recvMsgSize = recvfrom(sockfd, buffer, 8, 0, (struct sockaddr *)&server_address, &length);
            if(recvMsgSize <0)
            {
            ...
            fprintf("%d, %d\n", recvMsgSize, *buffer);
             ...
             buffer = calloc(1, *buffer+1);
            recvMsgSize = recvfrom(sockfd, buffer, recvMsgSize, 0, (struct sockaddr *)&server_address, &length);
            fprintf(stderr, "%d\n", recvMsgSize);
            if(recvMsgSize <0)
            {
                perror("Failed to Read");
