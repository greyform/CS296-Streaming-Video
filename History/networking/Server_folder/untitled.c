...
void* process_client(void* p) {
   ...
   // int window = cvNamedWindow("Webcam from Server", CV_WINDOW_NORMAL);
    ...
    IplImage* small = cvCreateImage(cvSize(frameORG->width/2, frameORG->height)/2, frameORG->depth, 3);
    ...
    senMsgSize = write_message_size(frameORG->width/2,childfd);
    senMsgSize = write_message_size(frameORG->height/2, childfd);
 ...
 CvVideoWriter * writer;
    ...
    while( 1){
       ...
       // cvResize(frameORG, small, CV_INTER_LINEAR );
        ...
        cvShowImage("Server", small); // feedback on server webcam
